...
class Connection
{
    ...
    public static function create(array $config): ConnectionInterface
    {
        ...
        // プレフィックスを各ドライバーの config にマージして渡す
        $prefix = ['prefix' => $config['prefix'] ?? 'tp_'];

        // ドライバー名に対応するクラスをインスタンス化して返す
        return match ($driver) {
            'sqlite' => new SQLiteDriver(array_merge($config['sqlite'], $prefix)),
            'mysql'  => new MySQLDriver(array_merge($config['mysql'], $prefix)),
            'pgsql'  => new PostgreSQLDriver(array_merge($config['pgsql'], $prefix)),
